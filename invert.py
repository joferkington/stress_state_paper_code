"""
Library for inverting fault slip data for principal stress directions using
Michael (1984)'s method.
"""
import numpy as np
...
def main():
    """Run a simple test if this file is run directly."""
...
def invert(normal, slip, weights=None):
    """
    Invert a set of slip vectors on faults for the stress state using 
    Michael (1984)'s methods.
    
    Parameters:
        normal: a 3xN array (or sequence of 3 arrays)
            The x,y,z components of the normal vectors of the fault planes.
        slip: A 3xN array (or sequence of 3 arrays)
            The x,y,z components of slip vectors along each fault plane.
        weights: An N-length numpy array (optional)
            The relative weight to be placed on each measurement.

    Returns:
        sigma: A 3x3 numpy array
            The normalized deaviatoric stress tensor.
    """
    nx, ny, nz = normal
    sx, sy, sz = slip
...
def invert_plane_stress(normal, slip, weights=None):
    """Invert a set of slip vectors on faults for the stress state using 
    Michael (1984)'s methods with a simplifying assumption for plane stress.
...
def principal(sigma):
    """
    Calculate the principal stress directions of a given stress tensor.

    Parameters:
        sigma: A 3x3 numpy array
            The input stress tensor
    Returns:
        vals: A 3-length numpy array
            The eigenvalues of the stress tensor from sigma1 to sigma3.
        vec_orientations: A 3-element list
            A list of tuples of plunge, bearing of the principal stress 
            directions derived from the eigenvectors of the stress tensor.
            The list is ordered from sigma1 to sigma3.
    """
